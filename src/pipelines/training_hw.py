...
class ModelAdapter:
    """Adapter to wrap different model types with a common interface."""

    def __init__(self, model, model_selection) -> None:  # noqa: D107
        self.model = model
        self.model_type = model_selection

    def fit(self, X, y, epochs = None, batch_size = None, verbose = 0):  # noqa: N803
        """Unifies the `.fit` API so Keras and sklearn models can be used interchangeably."""  # noqa: E501
        # Keras models have `fit` with epochs + batch_size
        if self.model_type == "sequential_nn":
        ...
        # sklearn/xgboost models ignore epochs & batch_size
        self.model.fit(X, y)

        # compute training_accuracy
        y_pred = self.model.predict(X)
        if y_pred.ndim > 1: # e.g. XGBoost logits
            y_pred = y_pred.argmax(axis=1)

        train_accuracy = accuracy_score(y, y_pred)

        return {
            "loss": float("nan"), #sklearn/xgboost don't track loss
    ...
    def evaluate(self, X, y, verbose=0):  # noqa: D102
        if self.model_type == "sequential_nn":
        ...
        # ensure y_pred and y are 1D arrays of labels
    ...
    def save(self, path):  # noqa: D102
        if self.model_type == "sequential_nn":
            ...
            if not path.endswith(".keras"):
                msg = "For 'sequential_nn' model, the path must end with '.keras' extension."  # noqa: E501
            ...
            if not path.endswith(".json"):
                msg = "For 'xgboost' model, the path must end with '.json' extension."
                raise ValueError(msg)
            ...
            if not path.endswith(".joblib"):
                msg = "For 'sklearn' model, the path must end with '.joblib' extension."
                raise ValueError(msg)

            dump(self.model, path)

    def get_extension(self):
        """Get the appropriate file extension for the model type."""
        if self.model_type == "sequential_nn":
            return ".keras"
        elif self.model_type == "xgboost":
            return ".json"
...
def initialize_model(model_selection, input_shape = None, random_state=42):
    """Initialize the model based on the selected type."""
    from sklearn.ensemble import RandomForestClassifier
    from sklearn.linear_model import LogisticRegression
    from xgboost import XGBClassifier  # noqa: PLC0415

    if model_selection == "sequential_nn":
        ...
        base_model = XGBClassifier(n_jobs=1,tree_method="hist",random_state=random_state)
    else:
        raise ValueError(f"Unknown model selection: {model_selection}")

    return ModelAdapter(base_model, model_selection)
...
class Training(Pipeline):
    ...
    model_selection = Parameter(
        "model-selection",
        help="Model selection strategy to use during training. " \
        "Options: sequential_nn, logistic_regression, random_forest, xgboost",  # noqa: ISC002
        default="sequential_nn"
    )
    ...
    def start(self):
        ...
        # Validate model selection parameter
        valid_model_selections = [
            "sequential_nn",
            "logistic_regression",
            "random_forest",
            "xgboost",
        ...
        if self.model_selection not in valid_model_selections:
            raise ValueError(f"Invalid model selection: {self.model_selection}. Valid options are: {valid_model_selections}")
        self.logger.info("Using model selection: %s", self.model_selection)
    ...
    def evaluate_fold(self):
        ...
        if self.model_selection in ["sequential_nn", "xgboost"]:
            if self.model_selection == "sequential_nn":
            ...
            y_pred = self.model.predict(self.x_test)
            self.test_precision = float("nan")
            self.test_recall = float("nan")

        self.logger.info(
            "Fold %d - test_loss: %f - test_accuracy: %f",
            self.fold,
            self.test_loss if self.test_loss is not None else float("nan"),
            self.test_accuracy,
        )
        # self.x = int('2fail')  # To trigger a failure for testing purposes
        # can use uv run src/piplines/training_hw.py resume {step_name} to resume from step
        # or failure step if not supplied

        # Let's track the evaluation metrics under the nested MLflow run corresponding
        # to the current fold.
        mlflow.log_metrics(
            {
                "test_loss": self.test_loss if self.test_loss is not None else float("nan"),
        ...
        # Create a card that shows confusion matrix
        cm = sk_confusion_matrix(self.y_test, y_pred)
        disp = ConfusionMatrixDisplay(confusion_matrix=cm)
        fig, ax = plt.subplots()
        disp.plot(ax=ax)
        current.card.append(
            Image.from_matplotlib(fig, label="Confusion Matrix")
        )
    ...
    def average_scores(self, inputs):
        ...
        test_accuracy_list = [i.test_accuracy for i in inputs]
        test_loss_list = [i.test_loss for i in inputs]
        self.test_accuracy = np.mean(test_accuracy_list)
        self.test_accuracy_std = np.std(test_accuracy_list)

        if self.model_selection == "sequential_nn":
            self.test_loss = np.mean(test_loss_list)
            self.test_loss_std = np.std(test_loss_list)
        else:
            self.test_loss = float("nan")
            self.test_loss_std = float("nan")
    ...
    def train(self):
        ...
        # Log system metrics
        print(mlflow.MlflowClient().get_run(self.mlflow_run_id).data)
    ...
    def _get_model_artifacts(self, directory: str):
        ...
        # Depending on the model, the extension will be different. .keras/.json/.joblib
        extension = self.model.get_extension()
        model_path = (Path(directory) / f"model{extension}").as_posix()
